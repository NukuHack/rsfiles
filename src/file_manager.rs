...
impl FileManager {
    ...
    fn handle_navigate_back(&mut self) -> Command<Message> {
        if let Some(_path) = self.navigation.go_back() {
    ...
    fn handle_navigate_forward(&mut self) -> Command<Message> {
        if let Some(_path) = self.navigation.go_forward() {
            self.refresh_directory()
        } else {
            Command::none()
        }
    ...
    fn view_file_row(&self, file: FileEntry) -> Element<Message> {
        ...
        let (prefix, text_color) = self.get_file_display_info(&file);
    ...
    fn get_file_display_info(&self, file: &FileEntry) -> (&'static str, iced::Color) {
        if file.is_dir() {
            ("[DIR]", iced::Color::from_rgb(0.5, 0.7, 1.0))
        } else if file.is_shortcut() {
            ("[LNK]", iced::Color::from_rgb(1.0, 0.8, 0.5))
        } else {
